...
def test_circular_calendar_returns_figure(blocos_df):
    fig = viz.circular_calendar(
        blocos_df, date="event_date", size="estimated_audience", color="region",
    )
    assert isinstance(fig, Figure)


def test_circular_calendar_converts_dates_to_angles(blocos_df):
    fig = viz.circular_calendar(
        blocos_df, date="event_date", size="estimated_audience", color="region",
    )
...
def test_circular_calendar_handles_missing_values(blocos_df):
    df = blocos_df.copy()
...
def test_circular_calendar_rejects_invalid_dates(blocos_df):
    df = blocos_df.copy()
...
def test_circular_calendar_rejects_missing_column(blocos_df):
    with pytest.raises(KeyError):
        viz.circular_calendar(
            blocos_df, date="not_a_column", size="estimated_audience", color="region",
        )


def test_circular_calendar_does_not_modify_input(blocos_df):
